...
export async function validateWorkspacePath(workspacePath: string): Promise<void> {
	...
	try {
		const testFile = path.join(workspacePath, ".mayai_write_test")
...
export async function initializeGitRepository(workspacePath: string): Promise<boolean> {
	...
	try {
		...
		await execa("git", ["config", "user.name", "Mayai Evaluation"], { cwd: workspacePath })
		await execa("git", ["config", "user.email", "mayai@example.com"], { cwd: workspacePath })
