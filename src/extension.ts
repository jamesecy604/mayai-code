...
export function activate(context: vscode.ExtensionContext) {
	...
	vscode.commands.executeCommand("setContext", "mayai.isDevMode", IS_DEV && IS_DEV === "true")
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("mayai.plusButtonClicked", async (webview: any) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("mayai.mcpButtonClicked", (webview: any) => {
	...
	const openMayaiInNewTab = async () => {
		...
		const panel = vscode.window.createWebviewPanel(WebviewProvider.tabPanelId, "Mayai", targetCol, {
			enableScripts: true,
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("mayai.settingsButtonClicked", (webview: any) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("mayai.historyButtonClicked", (webview: any) => {
			WebviewProvider.getAllInstances().forEach((instance) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("mayai.accountButtonClicked", (webview: any) => {
			WebviewProvider.getAllInstances().forEach((instance) => {
	...
	*/
	...
	if (IS_DEV && IS_DEV === "true") {
		...
		import("./dev/commands/tasks")
			.then((module) => {
				...
				Logger.log("Mayai dev task commands registered")
			})
			.catch((error) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("mayai.addToChat", async (range?: vscode.Range, diagnostics?: vscode.Diagnostic[]) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("mayai.addTerminalOutputToChat", async () => {
		...
		vscode.languages.registerCodeActionsProvider(
			...
			new (class implements vscode.CodeActionProvider {
				...
				provideCodeActions(
					...
					const addAction = new vscode.CodeAction("Add to Mayai", vscode.CodeActionKind.QuickFix)
					addAction.command = {
					...
					const fixAction = new vscode.CodeAction("Fix with Mayai", vscode.CodeActionKind.QuickFix)
					fixAction.command = {
						command: "mayai.fixWithMayai",
						title: "Fix with Mayai",
						arguments: [expandedRange, context.diagnostics],
					}
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("mayai.fixWithMayai", async (range: vscode.Range, diagnostics: vscode.Diagnostic[]) => {
			// Add this line to focus the chat input first
			await vscode.commands.executeCommand("mayai.focusChatInput")
			...
			const visibleWebview = WebviewProvider.getVisibleInstance()
			await visibleWebview?.controller.fixWithMayai(selectedText, filePath, languageId, diagnostics)
		}),
	)

	// Register the focusChatInput command handler
	context.subscriptions.push(
		vscode.commands.registerCommand("mayai.focusChatInput", () => {
			let visibleWebview = WebviewProvider.getVisibleInstance()
			if (!visibleWebview) {
				vscode.commands.executeCommand("mayai.SidebarProvider.focus")
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("mayai.generateGitCommitMessage", async () => {
			...
			if (controller) {
				...
				const outputChannel = vscode.window.createOutputChannel("Mayai Commit Generator")
	...
	return createMayaiAPI(outputChannel, sidebarWebview.controller)
}
