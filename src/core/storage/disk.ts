...
export async function ensureRulesDirectoryExists(): Promise<string> {
	...
	const mayaiRulesDir = path.join(userDocumentsPath, "Mayai", "Rules")
	try {
		await fs.mkdir(mayaiRulesDir, { recursive: true })
	} catch (error) {
		return path.join(os.homedir(), "Documents", "Mayai", "Rules") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
	return mayaiRulesDir
}
...
export async function getSavedMayaiMessages(context: vscode.ExtensionContext, taskId: string): Promise<MayaiMessage[]> {
...
export async function saveMayaiMessages(context: vscode.ExtensionContext, taskId: string, uiMessages: MayaiMessage[]) {
	try {
