...
describe("MayaiIgnoreController", () => {
	...
	describe("Include Directive", () => {
		it("should load patterns from an included file", async () => {
			...
			// Create a .mayaiignore file that includes .gitignore and adds an extra pattern "secret.txt"
			await fs.writeFile(path.join(tempDir, ".mayaiignore"), ["!include .gitignore", "secret.txt"].join("\n"))

			// Initialize the controller to load the updated .mayaiignore
			controller = new MayaiIgnoreController(tempDir)
			await controller.initialize()
			...
			// "secret.txt" should be ignored as specified directly in .mayaiignore
		...
		it("should handle non-existent included file gracefully", async () => {
			// Create a .mayaiignore file that includes a non-existent file
			await fs.writeFile(path.join(tempDir, ".mayaiignore"), ["!include missing-file.txt"].join("\n"))
		...
		it("should handle non-existent included file gracefully alongside a valid pattern", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".mayaiignore"), ["!include non-existent.txt", "*.tmp"].join("\n"))
