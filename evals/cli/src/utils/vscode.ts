...
/**
 * Spawn a VSCode instance with the Mayai extension
...
export async function spawnVSCode(workspacePath: string, vsixPath?: string): Promise<VSCodeResources> {
	...
	if (!vsixPath) {
		try {
			...
			const mayaiRoot = path.resolve(process.cwd(), "..", "..")
	...
	const tempUserDataDir = path.join(os.tmpdir(), `vscode-mayai-eval-${Date.now()}`)
	...
	const tempExtensionsDir = path.join(os.tmpdir(), `vscode-mayai-eval-ext-${Date.now()}`)
	...
	fs.writeFileSync(
		evalsEnvPath,
		`# This file activates Mayai test mode
# Created at: ${new Date().toISOString()}
# 
# This file is automatically detected by the Mayai extension
...
`,
	...
	// and configure Mayai to auto-open on startup
	...
	const settings = {
		...
		// Auto-open Mayai on startup
		"mayai.autoOpenOnStartup": true,

		// Show the activity bar and sidebar
		"workbench.activityBar.visible": true,
		"workbench.sideBar.visible": true,
		"workbench.view.extension.mayai.mayai-ActivityBar.visible": true,
	...
	console.log(`Created settings.json to disable workspace trust and auto-open Mayai`)

	// Create keybindings.json to automatically open Mayai on startup
	const keybindingsPath = path.join(settingsDir, "keybindings.json")
	const keybindings = [
		{
			key: "alt+c",
			command: "workbench.view.extension.mayai.mayai-ActivityBar",
			when: "viewContainer.workbench.view.extension.mayai.mayai-ActivityBar.enabled",
		},
		{
			key: "alt+shift+c",
	...
	console.log(`Created keybindings.json to help with Mayai activation`)
	...
	const args = [
		...
		"--start-up-extension",
		"mayai.mayai",
		// Run a command on startup to open Mayai
		"--command",
		"workbench.view.extension.mayai.mayai-ActivityBar",
	...
	const startupScript = `
		...
		setTimeout(() => {
			// Try to open Mayai in the sidebar
			require('vscode').commands.executeCommand('workbench.view.extension.mayai.mayai-ActivityBar');
			
			// Also try to open Mayai in a tab as a fallback
			setTimeout(() => {
				require('vscode').commands.executeCommand('mayai.openInNewTab');
			}, 5000);
		}, 5000);
	`
	fs.writeFileSync(startupScriptPath, startupScript)
	console.log(`Created startup script to activate Mayai`)
	...
	try {
		...
		const extensionDir = path.join(tempExtensionsDir, "mayai-activator")
		fs.mkdirSync(extensionDir, { recursive: true })

		// Create package.json for the extension
		const packageJsonPath = path.join(extensionDir, "package.json")
		const packageJson = {
			name: "mayai-activator",
			displayName: "Mayai Activator",
			description: "Activates Mayai and starts the test server",
			version: "0.0.1",
			engines: {
				vscode: "^1.60.0",
			},
			...
			contributes: {
				commands: [
					{
						command: "mayai-activator.activate",
						title: "Activate Mayai",
					},
				],
			},
		}
		...
		const extensionJs = `
			...
			function activate(context) {
				console.log('Mayai Activator is now active!');
				
				// Register the command to activate Mayai
				let disposable = vscode.commands.registerCommand('mayai-activator.activate', async function () {
					try {
						// Make sure the Mayai extension is activated
						const extension = vscode.extensions.getExtension('mayai.mayai');
						if (!extension) {
							console.error('Mayai extension not found');
							return;
						}
						
						if (!extension.isActive) {
							console.log('Activating Mayai extension...');
							await extension.activate();
						}
						
						// Show the Mayai sidebar
						console.log('Opening Mayai sidebar...');
						await vscode.commands.executeCommand('workbench.view.extension.mayai.mayai-ActivityBar');
						
						// Wait a moment for the sidebar to initialize
						await new Promise(resolve => setTimeout(resolve, 2000));
						
						// Also open Mayai in a tab as a fallback
						console.log('Opening Mayai in a tab...');
						await vscode.commands.executeCommand('mayai.openInNewTab');
						...
						const mayaiRootPath = '${path.resolve(process.cwd(), "..", "..")}';
						const visibleWebview = require(path.join(mayaiRootPath, 'src', 'core', 'webview')).WebviewProvider.getVisibleInstance();
						if (visibleWebview) {
							require(path.join(mayaiRootPath, 'src', 'services', 'test', 'TestServer')).createTestServer(visibleWebview);
							console.log('Test server created successfully');
						} else {
							console.error('No visible webview instance found');
						}
					} catch (error) {
						console.error('Error activating Mayai:', error);
					}
				});
				...
				setTimeout(() => {
					vscode.commands.executeCommand('mayai-activator.activate');
				}, 5000);
			}
		...
		console.log(`Created Mayai Activator extension`)

		// Try multiple approaches to activate the extension
		let serverStarted = false

		// Create an activation script to run in VS Code
		const activationScriptPath = path.join(settingsDir, "activate-mayai.js")
		const activationScript = `
			// This script will be executed to activate Mayai and start the test server
			const vscode = require('vscode');
			
			// Execute the mayai-activator.activate command
			vscode.commands.executeCommand('mayai-activator.activate');
		`
		...
		try {
			console.log("Executing activation script to start Mayai and test server...")
			await execa(
				"code",
				[
					"--user-data-dir",
		...
		if (!serverStarted) {
			...
			console.log("You may need to manually open the Mayai extension in VS Code")
		}
