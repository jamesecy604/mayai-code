...
/**
 * Send a task to the Mayai test server
 * @param task The task description to send
 * @param apiKey Optional Mayai API key to use for the task
...
export async function sendTaskToServer(task: string, apiKey?: string): Promise<any> {
	...
	try {
		...
		if (error.code === "ECONNREFUSED") {
			throw new Error(
				"Could not connect to the test server. Make sure VSCode is running with the Mayai extension and the test server is active.",
			)
		}
