...
description: "This protocol is designed to streamline the development process of building MCP servers with Mayai."
---

> 🚀 **Build and share your MCP servers with the world.** Once you've created a great MCP server, submit it to the [Mayai MCP Marketplace](https://github.com/mayai/mcp-marketplace) to make it discoverable and one-click installable by thousands of developers.

## What Are MCP Servers?

Model Context Protocol (MCP) servers extend AI assistants like Mayai by giving them the ability to:
...
The heart of effective MCP server development is following a structured protocol. This protocol is implemented through a `.mayairules` file that lives at the **root** of your MCP working directory (/Users/your-name/Documents/Mayai/MCP).

### Using `.mayairules` Files

A `.mayairules` file is a special configuration that Mayai reads automatically when working in the directory where it's placed. These files:

- Configure Mayai's behavior and enforce best practices
- Switch Mayai into a specialized MCP development mode
...
Here's the complete MCP Server Development Protocol that should be placed in your `.mayairules` file:
...
When this `.mayairules` file is present in your working directory, Mayai will:
...
### 1. Create a `.mayairules` file (🚨 IMPORTANT)

First, add a `.mayairules` file to the root of your MCP working directory using the protocol above. This file configures Mayai to use the MCP development protocol when working in this folder.

### 2. Start a Chat with a Clear Description

Begin your Mayai chat by clearly describing what you want to build. Be specific about:
...
Mayai will automatically start in PLAN MODE, guiding you through the planning process:
...
One of the most effective ways to help Mayai build your MCP server is to share official API documentation right at the start:
...
Providing comprehensive API details (endpoints, authentication, data structures) significantly improves Mayai's ability to implement an effective MCP server.

## Understanding the Two Modes

### PLAN MODE

In this collaborative phase, you work with Mayai to design your MCP server:
...
Once planning is complete, Mayai helps implement the server:
...
<Frame>
	<img
		src="https://storage.googleapis.com/mayai_public_images/docs/assets/planning-phase.gif"
...
<Frame>
	<img
		src="https://storage.googleapis.com/mayai_public_images/docs/assets/building-mcp-plugin.gif"
...
Resources let your MCP servers expose data to Mayai without executing code. They're perfect for providing context like files, API responses, or database records that Mayai can reference during conversations.
...
Resources make your MCP servers more context-aware, allowing Mayai to access specific information without requiring you to copy/paste. For more information, refer to the [official documentation](https://modelcontextprotocol.io/docs/concepts/resources).
