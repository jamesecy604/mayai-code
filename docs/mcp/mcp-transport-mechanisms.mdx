...
description: "Learn about the two primary transport mechanisms for communication between Mayai and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases."
---

Model Context Protocol (MCP) supports two primary transport mechanisms for communication between Mayai and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (Mayai) spawns an MCP server as a child process
...
- **Locality**: Runs on the same machine as Mayai
...
- Single-client scenarios (one Mayai instance per server)
...
1. The client (Mayai) connects to the server's SSE endpoint via HTTP GET request
...
- **Remote Access**: Can be hosted on a different machine from your Mayai instance
...
STDIO servers run locally on the same machine as Mayai, which has several important implications:
...
- **Execution**: Starts and stops with Mayai (child process lifecycle)
...
- Start when needed by Mayai
- Not require network configuration
- Need to be installed alongside Mayai or via a package manager
...
## Configuring Transports in Mayai

For detailed information on configuring STDIO and SSE transports in Mayai, including examples, see [Configuring MCP Servers](/mcp/configuring-mcp-servers).
